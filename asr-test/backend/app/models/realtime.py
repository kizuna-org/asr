...
class RealtimeCTCDecoder(nn.Module):
    ...
    def decode_realtime(self, log_probs: torch.Tensor, threshold: float = -5.0) -> List[int]:
        """
        リアルタイムデコード（簡易版）
        ...
        logger.debug(f"Decoding realtime with threshold: {threshold}, log_probs shape: {log_probs.shape}")
        ...
        for t in range(log_probs.size(0)):
            ...
            max_probs_list.append(max_prob.item())
            max_chars_list.append(max_char.item())
        ...
        # サマリーをログ出力
        if max_probs_list:
            avg_prob = sum(max_probs_list) / len(max_probs_list)
            max_max_prob = max(max_probs_list)
            logger.info(f"Decode summary: detected={len(detected_chars)} chars, avg_prob={avg_prob:.3f}, max_prob={max_max_prob:.3f}, threshold={threshold}")
...
class RealtimeASRModel(BaseASRModel):
    ...
    def inference(self, waveform: torch.Tensor) -> str:
        ...
        推論処理（リアルタイム用）
        """
        logger.debug("Starting realtime model inference",
                   extra={"extra_fields": {"component": "model", "action": "inference_start",
                                         "waveform_shape": str(waveform.shape), "dtype": str(waveform.dtype)}})
        ...
        try:
            ...
            # CTCデコード
            log_probs = self.decoder(encoder_output)
            logger.debug(f"CTC log_probs shape: {log_probs.shape}")
            logger.debug(f"CTC log_probs min/max: {log_probs.min().item():.3f}/{log_probs.max().item():.3f}")

            # リアルタイムデコード（複数の閾値を試す）
            detected_chars = self.decoder.decode_realtime(log_probs[0])

            # 文字が検出されなかった場合、より緩い閾値で再試行
            if len(detected_chars) == 0:
                logger.info("No chars detected with default threshold, trying lower threshold")
                detected_chars = self.decoder.decode_realtime(log_probs[0], threshold=-10.0)

            logger.debug(f"Detected chars: {detected_chars[:10]}... (total: {len(detected_chars)})" if len(detected_chars) > 10 else f"Detected chars: {detected_chars}")

            # 後処理
            recognized_text = self._post_process_ctc_output(detected_chars)

            if recognized_text:
                logger.info(f"Inference result: '{recognized_text}' ({len(detected_chars)} chars detected)")
            else:
                logger.info("Inference completed but no text recognized")
