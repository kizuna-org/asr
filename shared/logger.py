...
class StructuredLogger:
    def __init__(self, component: Component, job_id: Optional[str] = None):
        ...
        self.job_id = job_id
        # R2 client setup (only if job_id is set)
        self.r2_client = None
        ...
        if job_id:
            ...
            if endpoint_url and access_key and secret_key and bucket_name:
    ...
    def _create_log_entry(
        ...
        if context:
            log_entry["context"] = context  # type: ignore

        if error:
            log_entry["error"] = error  # type: ignore

        if performance:
            log_entry["performance"] = performance  # type: ignore

        if tags:
            log_entry["tags"] = tags  # type: ignore

        return log_entry

    def _log(self, log_entry: Dict[str, Any]) -> str:
        """Output the log entry to stdout and R2 (if available)"""
        log_json = json.dumps(log_entry, separators=(",", ":"))
        print(log_json)
        # Send to R2 if possible
        if self.r2_client and self.bucket_name and self.job_id:
            try:
                log_key = f"{self.job_id}/app.log"
                # Get existing log content
                try:
                    response = self.r2_client.get_object(
                        Bucket=self.bucket_name, Key=log_key
                    )
                    existing_content = response["Body"].read().decode("utf-8")
                except Exception:
                    existing_content = ""
                # Append new log
                updated_content = existing_content + log_json + "\n"
                ...
                # Do not raise, just print error to stderr
                print(f"[logger.py] Failed to stream log to R2: {e}", file=sys.stderr)
        return log_json + "\n"
    ...
    def debug(
        ...
        log_entry = self._create_log_entry(
            LogLevel.DEBUG, message, context=context, tags=tags
        )
    ...
    def warn(
        ...
        """Log warning message"""
        log_entry = self._create_log_entry(
            LogLevel.WARN, message, context=context, tags=tags
        )
        return self._log(log_entry)
    ...
    def operation_start(
        ...
        return self.info(
            f"Starting operation: {operation}", context=op_context, tags=tags
        )
